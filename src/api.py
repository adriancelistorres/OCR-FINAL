...
def realizar_ocr():
    try:
        ...
        # Redimensionar imagen 1600x900

        # Dimensiones deseadas
        nueva_altura = 900
        nueva_anchura = 1600

        # Verifica las dimensiones de la imagen original
        altura_original, anchura_original = imagen_np.shape[:2]

        if altura_original >= nueva_altura and anchura_original >= nueva_anchura:
            print('Sin INTER_CUBIC')
            imagen_redimensionada = cv2.resize(imagen_np, (nueva_anchura, nueva_altura))
        else:
            # Redimensiona la imagen con interpolación bicúbica si la imagen es de menor tamaño
            print('Con INTER_CUBIC')
            imagen_redimensionada = cv2.resize(imagen_np, (nueva_anchura, nueva_altura), interpolation=cv2.INTER_CUBIC)

        # Guarda la imagen redimensionada
        output_path = "imagenR.png"
        ...
        # Varía según tamaño de imagen original
        if altura_original < nueva_altura and anchura_original < nueva_anchura:
            y_positionMul = 22
        else: 
            y_positionMul = 20
        ...
        if servicios_line != -1:
            y_position = servicios_line * y_positionMul  # Ajusta la cantidad de píxeles a recortar arriba
            x_position = 60  # Ajusta la cantidad de píxeles a recortar a la izquierda
            image_cropped = imagen_redimensionada[y_position:, x_position:]
        else:
            # Si no se encuentra "servicios," mantener la imagen original
            image_cropped = imagen_redimensionada
